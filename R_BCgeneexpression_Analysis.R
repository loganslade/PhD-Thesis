...
bc.km <- bc.tsurv %>% mutate(strata = ifelse(TFEB  < l, "LOW", 
                                                ifelse(between(bc.tsurv$TFEB, l, h), "MEDIAN",
                                                        "HIGH")))
...
a + geom_jitter(alpha = 0.1, colour = "red3", size = 0.75, width = 0.2)+
...
ggsave("box_tfeb_metabric_pam50_v3.pdf", device = "pdf", width = 4.88, height = 6.26, units = "in")
...
tcgabc.clin <- dplyr::inner_join(x=tcgabc.c, y=sample.2, by = c("sample" = "patient_id"))
...
#####BOX-plots#######
a <- tcgabc.clin %>% drop_na(ihc_er) %>% ggplot(aes(ihc_er, TFEB)) 
a + geom_jitter(alpha = 0.1, colour = "red3", size = 0.75, width = 0.2)+
  geom_boxplot(fill = "grey65", colour = "black",  
               outlier.shape = NA, alpha=0.75, 
               fatten = 1, lwd = 0.5, width = 0.5, notch = TRUE)+
...
ggsave("box_TFEB_tcga_ihcer_v4.pdf", device = "pdf", width = 4.88, height = 6.26, units = "in")
